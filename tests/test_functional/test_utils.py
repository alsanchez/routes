...
class TestUtils(unittest.TestCase):
    ...
    def test_url_for_with_nongen(self):
        ...
        eq_(b'/blog', url_for('/blog'))
        eq_(b'/blog?q=fred&q=here%20now', url_for('/blog', q=['fred', u('here now')]))
        eq_(b'/blog#here', url_for('/blog', anchor='here'))
    ...
    def test_url_for_with_unicode(self):
        ...
        eq_(b'/blog', url_for(controller='blog'))
        eq_(b'/blog/view/umulat', url_for(controller='blog', action='view', id=u('umulat')))
        eq_(b'/blog/view/umulat?other=%CE%B1%CF%83%CE%B4%CE%B3', 
    ...
    def test_url_for(self):
        ...
        for urlobj in [url_for, url]:
            eq_(b'/blog', urlobj(controller='blog'))
            eq_(b'/content', urlobj())
            eq_(b'https://www.test.com/viewpost', urlobj(controller='post', action='view', protocol='https'))
            eq_(b'http://www.test.org/content', urlobj(host='www.test.org'))
    ...
    def test_url_for_with_defaults(self):
        ...
        eq_(b'/blog/view/4', url_for())
        eq_(b'/post/index/4', url_for(controller='post'))
        eq_(b'/blog/view/2', url_for(id=2))
        eq_(b'/viewpost/4', url_for(controller='post', action='view', id=4))

        eq_(b'/blog/view/4', url.current())
        eq_(b'/post/index/4', url.current(controller='post'))
        eq_(b'/blog/view/2', url.current(id=2))
        eq_(b'/viewpost/4', url.current(controller='post', action='view', id=4))
        
        con.mapper_dict = {'controller':'blog','action':'view','year':2004}
        url = URLGenerator(con.mapper, {'wsgiorg.routing_args':((), con.mapper_dict)})
        
        eq_(b'/archive/2004/10', url_for(month=10))
        eq_(b'/archive/2004/9/2', url_for(month=9, day=2))
        eq_(b'/blog', url_for(controller='blog', year=None))

        eq_(b'/archive/2004/10', url.current(month=10))
        eq_(b'/archive/2004/9/2', url.current(month=9, day=2))
        eq_(b'/blog', url.current(controller='blog', year=None))
    ...
    def test_url_for_with_more_defaults(self):
        ...
        eq_(b'/archive/2004/9/2', url_for(month=9, day=2))
        eq_(b'/blog', url_for(controller='blog', year=None))
        eq_(b'/archive/2004', url_for())

        eq_(b'/archive/2004/10', url.current(month=10))
        eq_(b'/archive/2004/9/2', url.current(month=9, day=2))
        eq_(b'/blog', url.current(controller='blog', year=None))
        eq_(b'/archive/2004', url.current())
    ...
    def test_url_for_with_defaults_and_qualified(self):
        ...
        eq_(b'/blog/view/4', url_for())
        eq_(b'/post/index/4', url_for(controller='post'))
        eq_(b'http://www.example.com/blog/view/4', url_for(qualified=True))
        eq_(b'/blog/view/2', url_for(id=2))
        eq_(b'/viewpost/4', url_for(controller='post', action='view', id=4))

        eq_(b'/blog/view/4', url.current())
        eq_(b'/post/index/4', url.current(controller='post'))
        eq_(b'http://www.example.com/blog/view/4', url.current(qualified=True))
        eq_(b'/blog/view/2', url.current(id=2))
        eq_(b'/viewpost/4', url.current(controller='post', action='view', id=4))
        ...
        eq_(b'/post/index/4', url_for(controller='post'))
        eq_(b'http://www.example.com:8080/blog/view/4', url_for(qualified=True))
        
        eq_(b'/post/index/4', url.current(controller='post'))
        eq_(b'http://www.example.com:8080/blog/view/4', url.current(qualified=True))
    ...
    def test_with_route_names(self):
        ...
        for urlobj in [url, url_for]:
            eq_(b'/content/view', urlobj(controller='content', action='view'))
            eq_(b'/content', urlobj(controller='content'))
            eq_(b'/admin/comments', urlobj(controller='admin/comments'))
            eq_(b'/category', urlobj('category_home'))
            eq_(b'/category/food', urlobj('category_home', section='food'))
            eq_(b'/', urlobj('home'))
    ...
    def test_with_route_names_and_defaults(self):
        ...
        eq_(b'/building/wilma/port/alljacks', url_for())
        eq_(b'/', url_for('home'))
        eq_(b'/building/wilma/port/alljacks', url.current())
        eq_(b'/', url.current('home'))
    ...
    def test_with_route_names_and_hardcode(self):
        ...
        eq_(b'/building/wilma/port/alljacks', url_for())
        eq_(b'/', url_for('home'))
        eq_(b'/gallery/home_thumbnail.jpg', url_for('gallery_thumb', img_id='home'))
        eq_(b'/gallery/home_thumbnail.jpg', url_for('gallery', img_id='home'))

        eq_(b'/building/wilma/port/alljacks', url.current())
        eq_(b'/', url.current('home'))
        eq_(b'/gallery/home_thumbnail.jpg', url.current('gallery_thumb', img_id='home'))
        eq_(b'/gallery/home_thumbnail.jpg', url.current('gallery', img_id='home'))
        
        m.hardcode_names = True
        eq_(b'/gallery/home_thumbnail.jpg', url_for('gallery_thumb', img_id='home'))
        eq_(b'/gallery/home.jpg', url_for('gallery', img_id='home'))

        eq_(b'/gallery/home_thumbnail.jpg', url.current('gallery_thumb', img_id='home'))
        eq_(b'/gallery/home.jpg', url.current('gallery', img_id='home'))
        m.hardcode_names = False
    ...
    def test_redirect_to(self):
        ...
        eq_(b'/content/view', redirect_to.result)
        redirect_to(controller='content', action='lookup', id=4)
        eq_(b'/content/lookup/4', redirect_to.result)
        redirect_to(controller='admin/comments',action='splash')
        eq_(b'/admin/comments/splash', redirect_to.result)
        redirect_to('http://www.example.com/')
        eq_(b'http://www.example.com/', redirect_to.result)
        redirect_to('/somewhere.html', var='keyword')
        eq_(b'/somewhere.html?var=keyword', redirect_to.result)
    ...
    def test_redirect_to_with_route_names(self):
        ...
        eq_(b'/content', redirect_to.result)
        redirect_to(controller='admin/comments')
        eq_(b'/admin/comments', redirect_to.result)
        redirect_to('category_home')
        eq_(b'/category', redirect_to.result)
        redirect_to('category_home', section='food')
        eq_(b'/category/food', redirect_to.result)
        redirect_to('home')
        eq_(b'/', redirect_to.result)
    ...
    def test_static_route(self):
        ...
        url = URLGenerator(m, {})
        for urlobj in [url_for, url]:
            eq_(b'http://www.groovie.org/', urlobj('home'))
            eq_(b'http://www.groovie.org/?s=stars', urlobj('home', s='stars'))
            eq_(b'/content/view', urlobj(controller='content', action='view'))
            eq_(b'/nasa/images?search=all', urlobj('space', search='all'))
    ...
    def test_static_route_with_script(self):
        ...
        for urlobj in [url_for, url]:
            eq_(b'http://www.groovie.org/', urlobj('home'))
            eq_(b'http://www.groovie.org/?s=stars', urlobj('home', s='stars'))
            eq_(b'/webapp/content/view', urlobj(controller='content', action='view'))
            eq_(b'/webapp/nasa/images?search=all', urlobj('space', search='all'))
            eq_(b'http://example.com/webapp/nasa/images', urlobj('space', protocol='http'))
            eq_(b'http://example.com/webapp/login', urlobj('login', qualified=True))
    ...
    def test_static_route_with_vars(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'http://fred.groovie.org/index', urlobj('home', domain='fred', location='index'))
            eq_(b'http://fred.groovie.org/index?search=all', urlobj('home', domain='fred', location='index', search='all'))
            eq_(b'/webapp/nasa/images?search=all', urlobj('space', location='images', search='all'))
            eq_(b'http://example.com/webapp/nasa/images', urlobj('space', location='images', protocol='http'))
    ...
    def test_static_route_with_vars_and_defaults(self):
        ...
        eq_(b'http://routes.groovie.org/index', url_for('home', location='index'))
        eq_(b'http://fred.groovie.org/index', url_for('home', domain='fred', location='index'))
        eq_(b'http://routes.groovie.org/index?search=all', url_for('home', location='index', search='all'))
        eq_(b'http://fred.groovie.org/index?search=all', url_for('home', domain='fred', location='index', search='all'))
        eq_(b'/webapp/nasa/articles?search=all', url_for('space', location='articles', search='all'))
        eq_(b'http://example.com/webapp/nasa/articles', url_for('space', location='articles', protocol='http'))
        eq_(b'/webapp/nasa/images?search=all', url_for('space', search='all'))
        eq_(b'http://example.com/webapp/nasa/images', url_for('space', protocol='http'))
        
        assert_raises(GenerationException, url.current, 'home')
        assert_raises(GenerationException, url.current, 'home', domain='fred')
        eq_(b'http://routes.groovie.org/index', url.current('home', location='index'))
        eq_(b'http://fred.groovie.org/index', url.current('home', domain='fred', location='index'))
        eq_(b'http://routes.groovie.org/index?search=all', url.current('home', location='index', search='all'))
        eq_(b'http://fred.groovie.org/index?search=all', url.current('home', domain='fred', location='index', search='all'))
        eq_(b'/webapp/nasa/articles?search=all', url.current('space', location='articles', search='all'))
        eq_(b'http://example.com/webapp/nasa/articles', url.current('space', location='articles', protocol='http'))
        eq_(b'/webapp/nasa/images?search=all', url.current('space', search='all'))
        eq_(b'http://example.com/webapp/nasa/images', url.current('space', protocol='http'))
    ...
    def test_static_route_with_vars_and_requirements(self):
        ...
        eq_(b'http://fred.groovie.org/index', url_for('home', domain='fred', location='index'))
        eq_(b'http://bob.groovie.org/index', url_for('home', domain='bob', location='index'))
        eq_(b'http://fred.groovie.org/asdf', url_for('home', domain='fred', location='asdf'))
        eq_(b'/webapp/nasa/articles/2004/6', url_for('space', year='2004', month='6'))
        eq_(b'/webapp/nasa/articles/2004/12', url_for('space', year='2004', month='12'))
        eq_(b'/webapp/nasa/articles/89/6', url_for('space', year='89', month='6'))
        ...
        eq_(b'http://fred.groovie.org/index', url.current('home', domain='fred', location='index'))
        eq_(b'http://bob.groovie.org/index', url.current('home', domain='bob', location='index'))
        eq_(b'http://fred.groovie.org/asdf', url.current('home', domain='fred', location='asdf'))
        eq_(b'/webapp/nasa/articles/2004/6', url.current('space', year='2004', month='6'))
        eq_(b'/webapp/nasa/articles/2004/12', url.current('space', year='2004', month='12'))
        eq_(b'/webapp/nasa/articles/89/6', url.current('space', year='89', month='6'))
    ...
    def test_no_named_path(self):
        ...
        url = URLGenerator(m, {})
        for urlobj in [url_for, url]:
            eq_(b'http://www.google.com/search', urlobj('http://www.google.com/search'))
            eq_(b'http://www.google.com/search?q=routes', urlobj('http://www.google.com/search', q='routes'))
            eq_(b'/delicious.jpg', urlobj('/delicious.jpg'))
            eq_(b'/delicious/search?v=routes', urlobj('/delicious/search', v='routes'))
    ...
    def test_append_slash(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'/delicious.jpg', urlobj('/delicious.jpg'))
            eq_(b'/delicious/search?v=routes', urlobj('/delicious/search', v='routes'))
            eq_(b'/content/list/', urlobj(controller='/content', action='list'))
            eq_(b'/content/list/?page=1', urlobj(controller='/content', action='list', page='1'))
    ...
    def test_no_named_path_with_script(self):
        ...
        for urlobj in [url_for, url]:
            eq_(b'http://www.google.com/search', urlobj('http://www.google.com/search'))
            eq_(b'http://www.google.com/search?q=routes', urlobj('http://www.google.com/search', q='routes'))
            eq_(b'/webapp/delicious.jpg', urlobj('/delicious.jpg'))
            eq_(b'/webapp/delicious/search?v=routes', urlobj('/delicious/search', v='routes'))
    ...
    def test_route_filter(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'/content/view-3.html', urlobj(controller='content', action='view', id=3))
            eq_(b'/content/index-2.html', urlobj(controller='content', id=2))
        
            eq_(b'/archives/2005/10/5/happy', 
            ...
            eq_(b'/archives/2003/8/2/woopee', urlobj('archives', article=story))
            eq_(b'/archives/2004/12/20/default', urlobj('archives', article=empty))
    ...
    def test_with_ssl_environ(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'/content/view', urlobj(controller='content', action='view'))
            eq_(b'/content/index/2', urlobj(controller='content', id=2))
            eq_(b'https://nowhere.com/content', urlobj(host='nowhere.com', controller='content'))
            ...
            eq_(b'/content/index/2', urlobj(controller='content', id=2))
            eq_(b'https://nowhere.com/content', urlobj(host='nowhere.com', controller='content'))
            eq_(b'https://nowhere.com:8080/content', urlobj(host='nowhere.com:8080', controller='content'))
            eq_(b'http://nowhere.com/content', urlobj(host='nowhere.com', protocol='http', controller='content'))
            eq_(b'http://home.com/content', urlobj(host='home.com', protocol='http', controller='content'))
    ...
    def test_with_http_environ(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'/content/view', urlobj(controller='content', action='view'))
            eq_(b'/content/index/2', urlobj(controller='content', id=2))
            eq_(b'https://example.com/content', urlobj(protocol='https', controller='content'))
    ...
    def test_subdomains(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'/content/view/3', urlobj(controller='content', action='view', id=3))
            eq_(b'http://new.example.com/content', urlobj(controller='content', sub_domain='new'))
    ...
    def test_subdomains_with_exceptions(self):
        ...
        url = URLGenerator(m, self.con.environ)
        eq_(b'/content/view', url_for(controller='content', action='view'))
        eq_(b'/content/index/2', url_for(controller='content', id=2))
        eq_(b'/content/view', url(controller='content', action='view'))
        eq_(b'/content/index/2', url(controller='content', id=2))
        ...
        eq_(b'/content/view/3', url_for(controller='content', action='view', id=3))
        eq_(b'http://new.example.com/content', url_for(controller='content', sub_domain='new'))
        eq_(b'http://example.com/content', url_for(controller='content', sub_domain='www'))
        eq_(b'/content/view/3', url(controller='content', action='view', id=3))
        eq_(b'http://new.example.com/content', url(controller='content', sub_domain='new'))
        eq_(b'http://example.com/content', url(controller='content', sub_domain='www'))
        
        self.con.mapper_dict = {'sub_domain':'www'}
        self.con.environ.update({'wsgiorg.routing_args':((), self.con.mapper_dict)})
        url = URLGenerator(m, self.con.environ)
        
        eq_(b'http://example.com/content/view/3', url_for(controller='content', action='view', id=3))
        eq_(b'http://new.example.com/content', url_for(controller='content', sub_domain='new'))
        eq_(b'/content', url_for(controller='content', sub_domain='sub'))
        
        # This requires the sub-domain, because we don't automatically go to the existing match dict
        eq_(b'http://example.com/content/view/3', url(controller='content', action='view', id=3, sub_domain='www'))
        eq_(b'http://new.example.com/content', url(controller='content', sub_domain='new'))
        eq_(b'/content', url(controller='content', sub_domain='sub'))
    ...
    def test_subdomains_with_named_routes(self):
        ...
        for urlobj in [url_for, url]:
            ...
            eq_(b'/content/index/2', urlobj(controller='content', id=2))
            eq_(b'/category', urlobj('category_home'))
            eq_(b'http://new.example.com/category', urlobj('category_home', sub_domain='new'))
        ...
        eq_(b'/content/view/3', url_for(controller='content', action='view', id=3))
        eq_(b'http://joy.example.com/building/west/merlot/alljacks', 
            url_for('building', campus='west', building='merlot', sub_domain='joy'))
        eq_(b'http://example.com/category/feeds', url_for('category_home', section='feeds', sub_domain=None))

        eq_(b'/content/view/3', url(controller='content', action='view', id=3))
        eq_(b'http://joy.example.com/building/west/merlot/alljacks', 
            url('building', campus='west', building='merlot', sub_domain='joy'))
        eq_(b'http://example.com/category/feeds', url('category_home', section='feeds', sub_domain=None))
    ...
    def test_subdomains_with_ports(self):
        ...
        for urlobj in [url, url_for]:
            ...
            eq_(b'/content/view', urlobj(controller='content', action='view'))
            eq_(b'/category', urlobj('category_home'))
            eq_(b'http://new.example.com:8000/category', urlobj('category_home', sub_domain='new'))
            eq_(b'http://joy.example.com:8000/building/west/merlot/alljacks', 
    ...
    def test_subdomains_with_default(self):
        ...
        self.con.environ['HTTP_HOST'] = 'example.com:8000'
        eq_(b'/content/view', urlobj(controller='content', action='view'))
        eq_(b'http://cat.example.com:8000/category', urlobj('category_home'))
...
class TestUtilsWithExplicit(unittest.TestCase):
    ...
    def test_url_for(self):
        ...
        eq_(b'/blog/view/3', url_for(controller='blog', action='view', id=3))
        eq_(b'https://www.test.com/viewpost', url_for(controller='post', action='view', protocol='https'))
        eq_(b'http://www.test.org/content/view/2', url_for(host='www.test.org', controller='content', action='view', id=2))
    ...
    def test_url_for_with_defaults_and_qualified(self):
        ...
        eq_(b'http://www.example.com/blog/view/4', url_for(qualified=True, controller='blog', action='view', id=4))
        eq_(b'/viewpost/4', url_for(controller='post', action='view', id=4))
        ...
        self.con.environ = env
        assert_raises(Exception, url_for, controller='post')
        eq_(b'http://www.example.com:8080/blog/view/4', url_for(qualified=True, controller='blog', action='view', id=4))
    ...
    def test_with_route_names(self):
        ...
        eq_(b'/category', url_for('category_home'))
        eq_(b'/category/food', url_for('category_home', section='food'))
    ...
    def test_with_route_names_and_nomin(self):
        ...
        assert_raises(Exception, url_for, controller='content')
        assert_raises(Exception, url_for, controller='admin/comments')
        eq_(b'/category/home', url_for('category_home'))
        eq_(b'/category/food', url_for('category_home', section='food'))
        assert_raises(Exception, url_for, 'home', controller='content')
        eq_(b'/', url_for('home'))
    ...
    def test_with_route_names_and_defaults(self):
        ...
        self.con.mapper_dict = dict(controller='building', action='showjacks', campus='wilma', building='port')
        assert_raises(Exception, url_for)
        eq_(b'/building/wilma/port/alljacks', url_for(controller='building', action='showjacks', campus='wilma', building='port'))
        eq_(b'/', url_for('home'))
    ...
    def test_with_resource_route_names(self):
        ...
        eq_(b'/messages/4', url_for('message', id=4))
        eq_(b'/messages/4/edit', url_for('edit_message', id=4))
        eq_(b'/messages/4/mark', url_for('mark_message', id=4))
        ...
        eq_(b'/messages/4.xml', url_for('formatted_message', id=4, format='xml'))
        eq_(b'/messages/4/edit.xml', url_for('formatted_edit_message', id=4, format='xml'))
        eq_(b'/messages/4/mark.xml', url_for('formatted_mark_message', id=4, format='xml'))
        eq_(b'/messages/new.xml', url_for('formatted_new_message', format='xml'))
